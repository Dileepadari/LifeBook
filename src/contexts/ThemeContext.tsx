...
// LifeBook, unlike workos's ThemeContext this is adapted from) -
...
export const colorPalettes: Record<Exclude<ColorPalette, 'custom'>, Record<string, string>> = {
  ...
  // The default. primary is the ADK Dev brand violet (#47266B) itself rather
  // than a generic purple, so the app shell and the logo mark agree.
  violet: {
    primary: hexToHSL('#47266B'), accent: hexToHSL('#a21caf'),
    darkPrimary: hexToHSL('#b39ddb'), darkAccent: hexToHSL('#e879f9'),
  },
  rose: {
...
const DEFAULT_CUSTOM_PRIMARY = '#47266B';
const DEFAULT_CUSTOM_ACCENT = '#a21caf';

export function ThemeProvider({ children }: { children: ReactNode }) {
  const [theme, setTheme] = useState<Mode>(() => {
    const stored = localStorage.getItem('lifebook-theme');
    if (stored === 'light' || stored === 'dark') return stored;
    return window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
  });
  const [colorPalette, setColorPaletteState] = useState<ColorPalette>(() => {
    const stored = localStorage.getItem('lifebook-palette');
    return PALETTE_IDS.includes(stored as ColorPalette) ? (stored as ColorPalette) : 'violet';
  });
  const [customPrimaryHex, setCustomPrimaryHex] = useState(() => localStorage.getItem('lifebook-custom-primary') || DEFAULT_CUSTOM_PRIMARY);
  const [customAccentHex, setCustomAccentHex] = useState(() => localStorage.getItem('lifebook-custom-accent') || DEFAULT_CUSTOM_ACCENT);
  ...
  useEffect(() => {
    ...
    localStorage.setItem('lifebook-theme', theme);
    applyColors();
  ...
  const setColorPalette = (palette: ColorPalette) => {
    ...
    localStorage.setItem('lifebook-palette', palette);
  };
  ...
  const setCustomColors = (primaryHex: string, accentHex: string) => {
    ...
    localStorage.setItem('lifebook-palette', 'custom');
    localStorage.setItem('lifebook-custom-primary', primaryHex);
    localStorage.setItem('lifebook-custom-accent', accentHex);
  };
