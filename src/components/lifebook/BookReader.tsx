 ...
 * The LifeBook reader.
 *
 * The product's claim is that a year of your life becomes a book, so the turn
...
function LeafContent({ page, pageNumber }: { page: LifePageType | null; pageNumber: number }) {
  if (!page) {
    ...
    return (
      <div className="flex h-full flex-col items-center justify-center bg-paper p-8 text-center">
        <p className="font-display text-sm tracking-[0.3em] text-paper-foreground/30 uppercase">
          {pageNumber <= 0 ? 'LifeBook' : 'Unwritten'}
        </p>
      </div>
    );
  }
